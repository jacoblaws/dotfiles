...
in
{
  flake.nixosModules.utl = { lib, ... }: {
    _module.args.utl = {
      fs = rec {
        dirModules = dir: b.readDir dir |> b.attrNames |> b.map (name: "${dir}/${name}");

        dirModulesRec = dir: lib.filesystem.listFilesRecursive dir;

        paths = f: dirs: b.map f dirs |> b.concatLists;
        ...
        genFiles =
          ...
          let
            ...
            attrValues =
              ...
              in
              b.concatMap genSpec themeNames;

            toAttrSet = name: value: { inherit name value; };
            zipped = lib.zipListsWith toAttrSet attrPaths attrValues;
            attrsList = b.map (set: lib.attrsets.setAttrByPath set.name set.value) zipped;
          in
          {
            # home.file = attrsets.mergeAttrsList attrsList;
            hjem.users.jvl.file = lib.attrsets.mergeAttrsList attrsList;
